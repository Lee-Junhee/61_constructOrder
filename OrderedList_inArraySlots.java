...
public class OrderedList_inArraySlots implements OrderedList {
  ...
  private int champIndex(ArrayList<Integer> challengers) {
    int champ = -1;
    for (int challenger = 0; challenger < challengers.size(); challenger++) {
      if (challengers.get(challenger) != null) {
        if (champ < 0 || challengers.get(champ) == null || challengers.get(champ) > challengers.get(challenger)) {
          champ = challenger;
        }
      }
    }
